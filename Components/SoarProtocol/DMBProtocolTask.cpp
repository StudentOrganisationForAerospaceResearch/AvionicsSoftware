...
void DMBProtocolTask::HandleProtobufCommandMessage(
  ...
  switch (msg.get_dmb_command().get_command_enum()) {
    ...
    case Proto::DMBCommand::Command::RSC_POWER_TRANSITION_ONBOARD:
      FlightTask::Inst().SendCommand(
          Command(CONTROL_ACTION, (uint16_t)RSC_POWER_TRANSITION_ONBOARD));
      break;
    case Proto::DMBCommand::Command::RSC_POWER_TRANSITION_EXTERNAL:
      FlightTask::Inst().SendCommand(
          Command(CONTROL_ACTION, (uint16_t)RSC_POWER_TRANSITION_EXTERNAL));
      break;
  ...
  if (msg.has_hb()) {
    ...
    if (msg.get_sys_ctrl().get_sys_cmd() ==
      ...
      SOAR_ASSERT(false, "System reset requested!");
               ...
               Proto::SystemControl::Command::SYS_CRITICAL_FLASH_FULL_ERASE) {
