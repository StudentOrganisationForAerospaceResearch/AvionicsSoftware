...
constexpr uint8_t FLIGHT_TASK_QUEUE_DEPTH_OBJS =
    10;  // Size of the flight task queue
constexpr uint16_t FLIGHT_TASK_STACK_DEPTH_WORDS =
    512;  // Size of the flight task stack

constexpr uint16_t FLIGHT_PHASE_DISPLAY_FREQ =
    1000;  // Display frequency for flight phase information
...
// BAROMETER TASK
constexpr uint8_t TASK_BAROMETER_PRIORITY =
    2;  // Priority of the barometer task
constexpr uint8_t TASK_BAROMETER_QUEUE_DEPTH_OBJS =
    10;  // Size of the barometer task queue
constexpr uint16_t TASK_BAROMETER_STACK_DEPTH_WORDS =
    512;  // Size of the barometer task stack

// IMU TASK (ACCEL/GYRO/MAGNETO)
constexpr uint8_t TASK_IMU_PRIORITY = 2;  // Priority of the barometer task
constexpr uint8_t TASK_IMU_QUEUE_DEPTH_OBJS =
    10;  // Size of the barometer task queue
constexpr uint16_t TASK_IMU_STACK_DEPTH_WORDS =
    512;  // Size of the barometer task stack

// GPS TASK
constexpr uint8_t TASK_GPS_PRIORITY = 2;  // Priority of the barometer task
constexpr uint8_t TASK_GPS_QUEUE_DEPTH_OBJS =
    10;  // Size of the barometer task queue
constexpr uint16_t TASK_GPS_STACK_DEPTH_WORDS =
    896;  // Size of the barometer task stack

// FLASH Task
constexpr uint8_t FLASH_TASK_RTOS_PRIORITY = 2;  // Priority of the flash task
constexpr uint8_t FLASH_TASK_QUEUE_DEPTH_OBJS =
    10;  // Size of the flash task queue
constexpr uint16_t FLASH_TASK_STACK_DEPTH_WORDS =
    512;  // Size of the flash task stack

// WATCHDOG Task
constexpr uint8_t WATCHDOG_TASK_RTOS_PRIORITY =
    3;  // Priority of the watchdog task
constexpr uint8_t WATCHDOG_TASK_QUEUE_DEPTH_OBJS =
    10;  // Size of the watchdog task queue
constexpr uint16_t WATCHDOG_TASK_STACK_DEPTH_WORDS =
    512;  // Size of the watchdog task stack
...
constexpr uint16_t HDI_TASK_STACK_DEPTH_WORDS =
    256;  // Size of the HDI task stack

// TELEMETRY Task
constexpr uint8_t TELEMETRY_TASK_RTOS_PRIORITY =
    2;  // Priority of the telemetry task
constexpr uint8_t TELEMETRY_TASK_QUEUE_DEPTH_OBJS =
    10;  // Size of the telemetry task queue
constexpr uint16_t TELEMETRY_TASK_STACK_DEPTH_WORDS =
    512;  // Size of the telemetry task stack

// PRESSURE TRANSDUCER TASK
constexpr uint8_t TASK_PRESSURE_TRANSDUCER_PRIORITY =
    2;  // Priority of the pressure transducer task
constexpr uint8_t TASK_PRESSURE_TRANSDUCER_QUEUE_DEPTH_OBJS =
    10;  // Size of the pressure transducer task queue
constexpr uint16_t TASK_PRESSURE_TRANSDUCER_STACK_DEPTH_WORDS =
    512;  // Size of the pressure transducer task stack

// BATTERY VOLTAGE Task
constexpr uint8_t BATTERY_TASK_RTOS_PRIORITY =
    2;  // Priority of the battery voltage task
constexpr uint8_t BATTERY_TASK_QUEUE_DEPTH_OBJS =
    10;  // Size of the battery voltage task queue
constexpr uint16_t BATTERY_TASK_STACK_DEPTH_WORDS =
    512;  // Size of the battery voltage task stack

constexpr uint32_t TELEMETRY_DEFAULT_LOGGING_RATE_MS =
    500;  // Default logging delay for telemetry task
constexpr uint32_t TELEMETRY_MINIMUM_LOG_PERIOD_MS =
    50;  // (1000/50 = 20hz) The minimum log period / max log rate
...
constexpr uint16_t MAX_NUMBER_OF_COMMAND_ALLOCATIONS =
    100;  // Let's assume ~128B per allocation, 100 x 128B = 12800B = 12.8KB

// DEBUG
constexpr uint16_t DEBUG_TAKE_MAX_TIME_MS =
    500;  // Max time in ms to take the debug semaphore
constexpr uint16_t DEBUG_SEND_MAX_TIME_MS =
    500;  // Max time the assert fail is allowed to wait to send header and message to HAL
constexpr uint16_t DEBUG_PRINT_MAX_SIZE =
    192;  // Max size in bytes of message print buffers

// ASSERT
constexpr uint16_t ASSERT_BUFFER_MAX_SIZE =
    160;  // Max size in bytes of assert buffers (assume x2 as we have two message segments)
constexpr uint16_t ASSERT_SEND_MAX_TIME_MS =
    250;  // Max time the assert fail is allowed to wait to send header and message to HAL (will take up to 2x this since it sends 2 segments)
constexpr uint16_t ASSERT_TAKE_MAX_TIME_MS =
    500;  // Max time in ms to take the assert semaphore
constexpr UARTDriver* const DEFAULT_ASSERT_UART_DRIVER =
