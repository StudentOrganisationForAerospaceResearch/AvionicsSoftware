...
bool FlashTask::ReadLogDataFromFlash() {
  ...
  for (unsigned int i = 0; i < currentOffsets_.writeDataOffset +
    ...
    if (length == sizeof(AccelGyroMagnetismData)) {
      ...
      W25qxx_ReadBytes(dataRead, SPI_FLASH_LOGGING_STORAGE_START_ADDR + i + 1,
                       sizeof(AccelGyroMagnetismData));
      AccelGyroMagnetismData* IMURead = (AccelGyroMagnetismData*)dataRead;
      SOAR_PRINT(
          "%03d %08d   %04d   %04d   %04d   %04d   %04d   %04d   %04d   %04d   "
          "%04d\n",
          length, IMURead->time, IMURead->accelX_, IMURead->accelY_,
          IMURead->accelZ_, IMURead->gyroX_, IMURead->gyroY_, IMURead->gyroZ_,
          IMURead->magnetoX_, IMURead->magnetoY_, IMURead->magnetoZ_);
