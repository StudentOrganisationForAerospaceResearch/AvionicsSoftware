...
void soar_assert_debug(bool condition, const char *file, const uint16_t line,
	...
	if (printMessage) {
		...
		int16_t res = snprintf(reinterpret_cast<char*>(header_buf),
		...
		HAL_UART_Transmit(DEFAULT_ASSERT_UART_HANDLE, header_buf,
				strlen(reinterpret_cast<char*>(header_buf)),
				ASSERT_SEND_MAX_TIME_MS);
		...
		if (printMessage && str != nullptr) {
			...
			if (buflen > 0) {
				str_buffer[buflen] = '\0';
				HAL_UART_Transmit(DEFAULT_ASSERT_UART_HANDLE, str_buffer,
						buflen, ASSERT_SEND_MAX_TIME_MS);
			}
		}
	} else {
		//TODO: Should manually print out the assertion header
		HAL_UART_Transmit(DEFAULT_ASSERT_UART_HANDLE,
