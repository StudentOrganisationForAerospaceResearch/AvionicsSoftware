...
void GPSTask::InitTask() {
  ...
  BaseType_t rtValue =
      xTaskCreate((TaskFunction_t)GPSTask::RunTask, (const char*)"GPSTask",
    ...
    if ((rx == '\r') || (rx == '\n'))  // End of line character has been reached
    {
              ...
              '$')  // Check that buffer has content and that the message is valid
      {
        ...
        if (gpsDataMutex.LockFromISR()) {
          memcpy(&data->buffer_, &rx_buffer,
                 rx_index);  // Copy to gps data buffer from rx_buffer
           ...
           NMEA_MAX_LENGTH))  // If start character received or end of rx buffer reached
      {
            ...
            6)  // If the first 6 characters follow $GPGGA, set gpggaDetected to 1
        {
          ...
          rx_buffer[rx_index++] =
            ...
            rx ==
            message[rx_index])  // Check if the first 6 characters follow $GPGGA
        {
          rx_buffer[rx_index++] = rx;
        } else {
      ...
      switch (counter) {
        ...
        case 4: {
          ...
          data->longitude_.minutes_ =
              (int32_t)((longitude - data->longitude_.degrees_ * 100) *
