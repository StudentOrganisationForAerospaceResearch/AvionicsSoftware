...
#define GET_COBS_MAX_LEN(len) \
    (((len) + ((len) / 254) + 1) + 1)  // Get the max length of a COBS encoded string, we add 1 for the 0x00 delimiter
...
// String Manipulation
inline bool IsAsciiNum(uint8_t c) {
    return (c >= '0' && c <= '9');
}
inline bool IsAsciiChar(uint8_t c) {
    return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z');
}
inline bool IsAsciiLowercase(uint8_t c) {
    return (c >= 'a' && c <= 'z');
}
